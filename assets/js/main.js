...
function injectMobileBottomBar() {
  ...
  const barHTML = `
    <nav id="mobileBottomBar" class="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-[#EAE3DA] py-1.5 px-2 flex items-center justify-around shadow-[0_-4px_20px_rgba(0,0,0,0.08)] mobile-quick-bar">
      
      <a href="index.html" class="flex flex-col items-center py-1 px-2 rounded-xl transition-colors tap-target ${isHome ? 'text-[#C85A32] font-bold' : 'text-[#574B46] hover:text-[#C85A32]'}">
      ...
      <a href="catalogo.html" class="flex flex-col items-center py-1 px-2 rounded-xl transition-colors tap-target ${isCat ? 'text-[#C85A32] font-bold' : 'text-[#574B46] hover:text-[#C85A32]'}">
        <svg class="w-5 h-5 mb-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <button class="open-cart-trigger relative flex flex-col items-center py-1 px-2 rounded-xl text-[#C85A32] tap-target">
        <div class="relative">
          <svg class="w-5 h-5 mb-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      ${isAdmin ? `
        <a href="admin.html" class="flex flex-col items-center py-1 px-2 rounded-xl transition-colors tap-target ${isAdminPage ? 'text-[#C85A32] font-black' : 'text-amber-600 font-bold hover:text-amber-700'}">
          <div class="relative">
            <svg class="w-5 h-5 mb-0.5 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
            <span class="absolute -top-1 -right-1 w-2 h-2 rounded-full bg-amber-500 animate-pulse"></span>
          </div>
          <span class="text-[10px] text-amber-700 font-extrabold">Admin</span>
        </a>
      ` : `
        <a href="javascript:void(0)" onclick="if(window.COMPANY_CONTACT && window.COMPANY_CONTACT.ENABLE_REDIRECTS){window.open(window.COMPANY_CONTACT.whatsapp.url_principal,'_blank');}else{if(window.showToast){showToast('Atención por WhatsApp: ' + (window.COMPANY_CONTACT?.whatsapp.principal || '+51 900 000 000') + ' (Pausado temporalmente)','info');}}" class="flex flex-col items-center py-1 px-2 rounded-xl text-emerald-600 tap-target" title="Atención comercial">
      ...
      <a href="${user ? 'perfil.html' : 'login.html'}" class="flex flex-col items-center py-1 px-2 rounded-xl transition-colors tap-target ${isProfile ? 'text-[#C85A32] font-bold' : 'text-[#574B46] hover:text-[#C85A32]'}">
        <svg class="w-5 h-5 mb-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
        </svg>
        <span class="text-[10px]">${user ? 'Cuenta' : 'Ingresar'}</span>
      </a>
