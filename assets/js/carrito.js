...
const Carrito = {
  ...
  renderDrawerMarkup() {
    ...
    const drawerHTML = `
      ...
      <aside id="cartDrawer" class="fixed top-0 right-0 h-full w-full max-w-[100vw] sm:max-w-md bg-[#FDFBF7] shadow-2xl z-50 transform translate-x-full transition-transform duration-300 ease-in-out flex flex-col border-l border-[#EAE3DA]">
        ...
        <div class="px-4 sm:px-6 py-3.5 sm:py-4 bg-white/95 backdrop-blur border-b border-[#EAE3DA] flex items-center justify-between flex-shrink-0">
          <div class="flex items-center gap-2.5 sm:gap-3">
            ...
            <div>
              ...
              <p class="text-[11px] text-[#574B46]">Atención comercial y cotizaciones formales</p>
            </div>
          </div>
          <button id="closeCartBtn" class="p-2 sm:p-2.5 rounded-xl text-stone-500 hover:text-stone-800 hover:bg-stone-100 transition-colors tap-target cursor-pointer" aria-label="Cerrar carrito">
        ...
        <!-- Cuerpo Desplazable: Lista de Productos + Formulario de Datos -->
        <div class="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 touch-scroll">
          <!-- Lista de Productos -->
          <div id="cartItemsList" class="space-y-3 sm:space-y-4">
            <!-- Renderizado dinámico -->
          </div>

          <!-- Card: Datos de Cotización (Ubicado en el cuerpo para no obstruir botones) -->
          <div id="cartCustomerCard" class="bg-[#F4EFEA] p-3.5 sm:p-4 rounded-2xl text-xs text-[#574B46] space-y-2.5 border border-[#EAE3DA]">
            <div class="font-semibold text-[#1F1815] flex items-center justify-between pb-1 border-b border-[#EAE3DA]">
              <span class="flex items-center gap-1.5 font-bold text-xs">
                <svg class="w-4 h-4 text-[#C85A32]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                <span>Datos del Cliente / Facturación</span>
              </span>
              <span class="text-[10px] font-normal text-[#C85A32]">*Requerido</span>
            </div>
            
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1">
              <div>
                <label class="block text-[11px] text-[#574B46] mb-0.5 font-medium">Comprobante:</label>
                <select id="cotizacionTipoComp" class="w-full text-xs bg-white border border-[#EAE3DA] rounded-xl px-2.5 py-2 focus:outline-none focus:border-[#C85A32]">
                  <option value="Factura">Factura con RUC</option>
                  <option value="Boleta">Boleta de Venta</option>
                </select>
              </div>
              <div>
                <label class="block text-[11px] text-[#574B46] mb-0.5 font-medium">RUC / DNI:</label>
                <input type="text" id="cotizacionDoc" placeholder="11 u 8 dígitos" class="w-full text-xs bg-white border border-[#EAE3DA] rounded-xl px-2.5 py-2 focus:outline-none focus:border-[#C85A32]" maxlength="11">
              </div>
            </div>

            <div>
              <label class="block text-[11px] text-[#574B46] mb-0.5 font-medium">Cliente / Razón Social:</label>
              <input type="text" id="cotizacionNombre" placeholder="Ej: Distribuidora Gastronómica / Juan Pérez" class="w-full text-xs bg-white border border-[#EAE3DA] rounded-xl px-2.5 py-2 focus:outline-none focus:border-[#C85A32]">
            </div>

            <div>
              <label class="block text-[11px] text-[#574B46] mb-0.5 font-medium">Destino / Agencia de Envíos:</label>
              <select id="cotizacionDestino" class="w-full text-xs bg-white border border-[#EAE3DA] rounded-xl px-2.5 py-2 focus:outline-none focus:border-[#C85A32]">
  ...
  renderCartItems() {
    ...
    const customerCard = document.getElementById('cartCustomerCard');
    ...
    if (this.items.length === 0) {
      ...
      `;
      if (customerCard) customerCard.classList.add('hidden');
